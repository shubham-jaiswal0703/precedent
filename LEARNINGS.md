...
- Scene `metadata` dict is indexable + filterable via `search(filter=[...])`, our legal-moment structured fields ride on this.
...
- Director (MIT) has ~27 agents incl. prompt_clip, comparison, subtitle: liftable.
...
`Segmenter.sentence` gives clean sentence boundaries (no speaker field: aggregate from word level). This unlocked:
